...
function render_background_scanline(fc, line, spp, buffer, buffer_offset) {
...
function expand_line_8(p0, p1, high, buffer, offset) {
    // 0 - D7
    const low0 = ((p0 & 0x80) >> 7) | ((p1 & 0x80) >> 6);
    palette_data[high] = buffer[offset + 0];
    buffer[offset + 0] = palette_data[high | low0];
    // 1 - D6
    const low1 = ((p0 & 0x40) >> 6) | ((p1 & 0x40) >> 5);
    palette_data[high] = buffer[offset + 1];
    buffer[offset + 1] = palette_data[high | low1];
    // 2 - D5
    const low2 = ((p0 & 0x20) >> 5) | ((p1 & 0x20) >> 4);
    palette_data[high] = buffer[offset + 2];
    buffer[offset + 2] = palette_data[high | low2];
    // 3 - D4
    const low3 = ((p0 & 0x10) >> 4) | ((p1 & 0x10) >> 3);
    palette_data[high] = buffer[offset + 3];
    buffer[offset + 3] = palette_data[high | low3];
    // 4 - D3
    const low4 = ((p0 & 0x08) >> 3) | ((p1 & 0x08) >> 2);
    palette_data[high] = buffer[offset + 4];
    buffer[offset + 4] = palette_data[high | low4];
    // 5 - D2
    const low5 = ((p0 & 0x04) >> 2) | ((p1 & 0x04) >> 1);
    palette_data[high] = buffer[offset + 5];
    buffer[offset + 5] = palette_data[high | low5];
    // 6 - D1
    const low6 = ((p0 & 0x02) >> 1) | ((p1 & 0x02) >> 0);
    palette_data[high] = buffer[offset + 6];
    buffer[offset + 6] = palette_data[high | low6];
    // 7 - D0
    const low7 = ((p0 & 0x01) >> 0) | ((p1 & 0x01) << 1);
    palette_data[high] = buffer[offset + 7];
    buffer[offset + 7] = palette_data[high | low7];
}


function expand_line_8_r(p0, p1, high, buffer, offset)  {
    // 7 - D7
    const low0 = ((p0 & 0x80) >> 7) | ((p1 & 0x80) >> 6);
    palette_data[high] = buffer[offset + 7];
    buffer[offset + 7] = palette_data[high | low0];
    // 6 - D6
    const low1 = ((p0 & 0x40) >> 6) | ((p1 & 0x40) >> 5);
    palette_data[high] = buffer[offset + 6];
    buffer[offset + 6] = palette_data[high | low1];
    // 5 - D5
    const low2 = ((p0 & 0x20) >> 5) | ((p1 & 0x20) >> 4);
    palette_data[high] = buffer[offset + 5];
    buffer[offset + 5] = palette_data[high | low2];
    // 4 - D4
    const low3 = ((p0 & 0x10) >> 4) | ((p1 & 0x10) >> 3);
    palette_data[high] = buffer[offset + 4];
    buffer[offset + 4] = palette_data[high | low3];
    // 3 - D3
    const low4 = ((p0 & 0x08) >> 3) | ((p1 & 0x08) >> 2);
    palette_data[high] = buffer[offset + 3];
    buffer[offset + 3] = palette_data[high | low4];
    // 2 - D2
    const low5 = ((p0 & 0x04) >> 2) | ((p1 & 0x04) >> 1);
    palette_data[high] = buffer[offset + 2];
    buffer[offset + 2] = palette_data[high | low5];
    // 1 - D1
    const low6 = ((p0 & 0x02) >> 1) | ((p1 & 0x02) >> 0);
    palette_data[high] = buffer[offset + 1];
    buffer[offset + 1] = palette_data[high | low6];
    // 0 - D0
    const low7 = ((p0 & 0x01) >> 0) | ((p1 & 0x01) << 1);
    palette_data[high] = buffer[offset + 0];
    buffer[offset + 0] = palette_data[high | low7];
}
...
function render_frame(fc, buffer) {
    ...
    // 精灵使用的图样板
    const spp = (fc.ppu.ctrl & PPU2000_SpTabl ? 4 : 0) * 1024;
    ...
    for (let i = 0; i !== SCAN_LINE_COUNT; ++i) {
        ...
        for (; fc.cpu_cycle_count < end_cycle_count_this_round;)
            fc.cpu_execute_one();
        // 渲染背景
        render_background_scanline(fc, i, spp, buffer, buffer_offset);
        buffer_offset += 256;
        // 执行HBlank
    }
    // 后渲染
    ...
    {
        for (let i = 0; i != 16; ++i) {
            palette_data[i] = fc.ppu.spindexes[i + 16];
        }
        palette_data[4 * 1] = palette_data[0];
        palette_data[4 * 2] = palette_data[0];
        palette_data[4 * 3] = palette_data[0];
    ...
    for (let i = 63; i !== -1; --i) {
        ...
        if (aa & SPATTR_FlipH) {
            for (let j = 0; j !== 8; ++j) {
                ...
                expand_line_8_r(p0, p1, high, buffer, xx + (yy + j + 1) * 256);
            }
        }
        else
            for (let j = 0; j !== 8; ++j) {
                let p0 = fc.ppu.banks[Math.floor((nowp0 + j) / 1024)][(nowp0 + j) % 1024];
                let p1 = fc.ppu.banks[Math.floor((nowp1 + j) / 1024)][(nowp1 + j) % 1024];
                expand_line_8(p0, p1, high, buffer, xx + (yy + j + 1) * 256);
            }
    }
